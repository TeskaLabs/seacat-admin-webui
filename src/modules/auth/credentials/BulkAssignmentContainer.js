			...
			customComponent: {
				generate: (obj) => (
					<div className="no-wrap-40ch">
						{(obj.suspended === true) ?
							<span className="cat-user-xmark text-muted me-1" title={(obj.registered === false) ? t("BulkAssignmentContainer|Credentials invited") : t("BulkAssignmentContainer|Credentials suspended")}/>
					...
					{selectedCredentials.map((obj, idx) => {
						return (
							<div className='d-flex flex-direction-row align-items-center selected-row'>
								...
								<i className="at-account me-1 ms-3"/>{obj.username ?? obj._id}
							</div>
						)
					})}
				</CardBody>
