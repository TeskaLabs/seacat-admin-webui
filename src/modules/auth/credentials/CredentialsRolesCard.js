...
function CredentialsRolesCard(props) {
	...
	const retrieveAssignedRoles = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("CredentialsRolesCard|Something went wrong, failed to fetch assigned roles", {error: e?.response?.data?.message}), 30);
		}
	};
	...
	const retrieveRolesLookup = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("CredentialsRolesCard|Something went wrong, failed to fetch roles", {error: e?.response?.data?.message}), 30);
		}
	};
	...
	const handleSave = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("CredentialsRolesCard|Failed to update roles", {error: e?.response?.data?.message}), 30);
		}
	}
