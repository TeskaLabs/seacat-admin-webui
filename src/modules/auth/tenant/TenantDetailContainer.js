...
function TenantDetailContainer(props) {
	...
	const retrieveData = async () => {
		try {
			...
			console.error(e);
			setLoadingCustomData(false);
			props.app.addAlert("warning", t("TenantDetailContainer|Something went wrong, failed to fetch tenant detail", {error: e?.response?.data?.message}), 30);
		}
	};
	...
	const retrieveUserName = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("TenantDetailContainer|Something went wrong, failed to fetch assigned credentials", {error: e?.response?.data?.message}), 30);
		}
	}
	...
	const removeTenant = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("TenantDetailContainer|Failed to remove the tenant", {error: e?.response?.data?.message}), 30);
		}
	}
	...
	const retrieveCredentialsForDropdown = async () => {
		...
		try {
			...
			props.app.addAlert("warning", t("TenantDetailContainer|Something went wrong, failed to fetch data", {error: e?.response?.data?.message}), 30);
		}
	};
	...
	const assignCredentials = async (credentialsId) => {
		try {
			...
			if (e.response?.data?.result === "ALREADY-EXISTS") {
				...
				console.error(e);
				props.app.addAlert("warning", t("TenantDetailContainer|Something went wrong, the credentials cannot be assigned", {error: e?.response?.data?.message}), 30);
			}
		}
	};
	...
	const unassignCredentials = async (credentialsId) => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("TenantDetailContainer|Something went wrong, the credentials cannot be unassigned", {error: e?.response?.data?.message}), 30);
		}
	};
