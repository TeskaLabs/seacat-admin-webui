...
const ClientDetailContainer = (props) =>  {
	...
	const getClientDetail = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("ClientDetailContainer|Something went wrong, failed to fetch client details", {error: e?.response?.data?.message}), 30);
		}
	}
	...
	const resetSecret = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t('ClientDetailContainer|Something went wrong, failed to reset secret', {error: e?.response?.data?.message}), 30);
		}
	}
	...
	const onSubmit = async (values) => {
		...
		try {
			...
			setEditMode(true);
			console.error(e);
			props.app.addAlert("warning", t("ClientDetailContainer|Something went wrong, failed to update client", {error: e?.response?.data?.message}), 30);
		}
	}
	...
	const removeClient = async () => {
		try {
			...
			props.app.addAlert("warning", t('ClientDetailContainer|Something went wrong, failed to remove client', {error: e?.response?.data?.message}), 30);
		}
	};
